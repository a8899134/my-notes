...
举例说明：
假设你在一台 4 核 8 GB 内存的服务器上运行了三个容器：A(Web 服务)、B(数据库)、C(缓存)。如果不对容器 A 做任何限制，当它因为某个异常(如内存泄露、遭受攻击)突然消耗 7 GB 内存时，宿主机只剩下 1 GB 内存给 B 和 C 使用。结果就是：
...
**关键结论**：`deploy.resources` 仅在 Swarm 模式下生效。如果你使用 `docker compose up` 启动容器，必须使用顶层的 `cpus`、`mem_limit` 字段。
## 5.2 单机模式下的写法
...
```
## 5.4 一份文件通吃两种模式
**生产环境** 同一份 `docker-compose.yml` 文件既能用 `docker compose up` 单机启动，也能用 `docker stack deploy` 部署到 Swarm 集群，可以两种写法都保留。
原理：
- 单机 docker compose up 会读取顶层的 cpus / mem_limit，忽略 deploy.resources。
- Swarm docker stack deploy 会读取 deploy.resources，忽略顶层的 cpus / mem_limit。
- 两者互不干扰，各自生效。
完整示例(双保险写法)：
```yaml
version: '3.8'
...
services:
  web:
    ...
    ports:
      - "8080:80"
    # 单机 docker compose up 读取这里
    cpus: '0.5'
    mem_limit: 256M
    mem_reservation: 128M
    # Swarm docker stack deploy 读取这里
    deploy:
      resources:
        limits:
          cpus: '0.5'
...
```
验证方法：
- 启动后执行 `docker inspect 容器名 --format='{{.HostConfig.Memory}}'`，如果返回了你设置的值(如 `268435456` 对应 256 M)，说明限制已生效。
- 如果返回 `0`，说明限制未生效，请检查你使用的是哪种启动命令。
-- -
## 六、生产环境资源限制最佳实践
**生产环境**：无论你用哪种方式启动，每个容器都必须设置内存上限(mem_limit 或 deploy.resources.limits.memory)，这是防止单个容器拖垮整台宿主机的最后一道防线。
### 6.1 各类型服务推荐配置
| 服务类型                           | CPU 上限    | 内存上限          | 说明                 |
| ------------------------------ | --------- | ------------- | ------------------ |
...
| 生产环境的 Compose 文件已配置资源限制(单机用 `cpus` / `mem_limit`，Swarm 用 `deploy.resources`) | ☐   |
