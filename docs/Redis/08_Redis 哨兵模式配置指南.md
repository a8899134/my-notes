...
### 4.4 配置文件中的注释特性
**重要说明：** sentinel.conf 文件中以 # 开头的行均为注释，表示该配置项未生效。例如 # sentinel auth-pass mymaster MySUPER--secret-0123passw0rd 被注释了，说明未配置认证密码。

如果 Redis 主节点设置了密码，必须在哨兵配置文件中取消注释并配置正确的密码，否则哨兵将无法连接 Redis 节点进行监控，故障转移功能会失效。
### 4.5 复制配置文件到其他节点
